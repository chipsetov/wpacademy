...
function core_upgrade_preamble() {
	...
	if ( $updates && ( count( $updates ) > 1 || $updates[0]->response != 'latest' ) ) {
		...
		echo '<p>' . sprintf( __( '<a href="%s">Learn more about WordPress %s</a>.' ), esc_url( self_admin_url( 'about-tamarillo.php' ) ), $normalized_version ) . '</p>';
	}
	dismissed_updates();
}
...
<?php
	...
	show_message( '<span class="hide-if-no-js">' . sprintf( __( 'Welcome to WordPress %1$s. You will be redirected to the About WordPress screen. If not, click <a href="%2$s">here</a>.' ), $result, esc_url( self_admin_url( 'about-tamarillo.php?updated' ) ) ) . '</span>' );
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to WordPress %1$s. <a href="%2$s">Learn more</a>.' ), $result, esc_url( self_admin_url( 'about-tamarillo.php?updated' ) ) ) . '</span>' );
	?>
	</div>
	<script type="text/javascript">
	window.location = '<?php echo self_admin_url( 'about-tamarillo.php?updated' ); ?>';
	</script>
	<?php
}
